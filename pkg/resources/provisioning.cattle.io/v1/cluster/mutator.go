...
func parseFromRawArgs(input interface{}) (keyValueArgs, error) {
	...
	if parsed == nil {
		return nil, fmt.Errorf("failed to convert input into slice: invalid type: %v", input)
	}
	args := keyValueArgs{}
...
func (m *ProvisioningClusterMutator) handlePSACT(request *admission.Request, cluster *v1.Cluster) (*admissionv1.AdmissionResponse, error) {
	...
	case admissionv1.Create, admissionv1.Update:
		...
		if templateName == "" {
			...
			args := getKubeAPIServerArgs(cluster)
...
func getKubeAPIServerArgs(cluster *v1.Cluster) keyValueArgs {
	...
	if err != nil {
		logrus.Errorf("error parsing kube-apiserver-arg: %v", err)
		return keyValueArgs{}
	}
	return args
}
